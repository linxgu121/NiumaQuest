...
NiumaQuest 按“一个任务一个资产”管理。

- `QuestAsset`：一个任务一个资产，例如找回铁剑、收集材料、拜访 NPC。
- 任务链或故事线不要塞进单个 `QuestAsset`，应由多个 QuestAsset 加 Story / Dialogue / Reward 串起来。
- 任务奖励后期推荐引用 `RewardPackageDefinition`；旧 QuestRewardData 可作为桥接兼容。

任务是否已接取、已完成、待领奖、已领奖属于运行时状态和存档，不写进 `QuestAsset`。
