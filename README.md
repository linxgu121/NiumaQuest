...
- `QuestRoot/UIBridge` 或 `UIRoot/Bridges`：挂 `QuestUIViewBridge`，绑定任务追踪 UI Receiver。UI Toolkit 场景优先使用 `QuestToolkitReceiver`。
...
- `UIRoot/QuestTracker`：旧 UGUI 场景放任务追踪面板；UI Toolkit 场景在注册表中配置 `QuestTracker` View，并由 `QuestToolkitReceiver` 接收 ViewData。
...
| `Receiver Provider` | UI Toolkit 场景拖 `QuestToolkitReceiver`；旧 UGUI 场景拖自制 `IQuestUIReceiver` | 不可以 | ViewData 无处显示 |
